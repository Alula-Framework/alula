...
public struct SwiftMetricsReporter: Sendable {
    ...
    ///   name. (Across events, `FlightTelemetryModule` refuses it earlier.)
    public func attach(
        _ metrics: [TelemetryMetric], id: String = "flight.telemetry.swift-metrics"
    ...
    /// A metric name as a swift-metrics label: `flight.sessions.created` is
    /// `flight_sessions_created`. Anything outside `[A-Za-z0-9_:]` becomes
