...
struct HTMLRenderingTests {
    ...
    func servesHTMLByDefault() async throws {
        ...
        #expect(response.bodyText.contains("<h1>Alula Actuator</h1>"))
    }
    ...
    func pageContents() async throws {
        ...
        #expect(body.contains("AlulaActuatorTests.SampleService"))
        #expect(body.contains("AlulaActuatorTests.SampleRepository"))
        // Layer grouping (Alula Core): non-empty sections render as
        ...
        // @Middleware and @Settings components get their own sections — added to
        ...
        #expect(body.contains("AlulaActuatorTests.SampleMiddleware"))
        #expect(body.contains("AlulaActuatorTests.SampleSettings"))
        // Two registrations of one type render as two rows, not one.
        let duplicatedRows = body.components(
            separatedBy: "AlulaActuatorTests.SampleDuplicated"
        ).count - 1
    ...
    func everyStereotypeHasASection() {
        ...
        // .component, the way a component of an unrecognized *type* might be —
        // its components simply never appear on the page at all, silently. That
        ...
        // failed until Aluladeck's own dashboard was read by eye. Reading
...
struct HTMLEscapingTests {
    ...
    func passthrough() {
        #expect(htmlEscaped("AlulaCore.Configuration") == "AlulaCore.Configuration")
        #expect(htmlEscaped("") == "")
