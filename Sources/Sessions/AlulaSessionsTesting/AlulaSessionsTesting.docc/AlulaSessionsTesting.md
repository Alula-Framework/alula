...
let client = try TestClient(
    routes: LoginController.alulaRoutes { _ in LoginController() },
    middleware: try AlulaSessionsModule(configuration: config, store: store).middleware)
