...
struct LifecycleTests {
    ...
    func explicitLeave() async throws {
        ...
        try wire.send(ref: "2", topic: "room:1", event: "alula:leave")
    ...
    func leaveUnjoined() async throws {
        ...
        try wire.send(ref: "1", topic: "room:1", event: "alula:leave")
        #expect(try await wire.nextEnvelope()?.payload == ["reason": "not_joined"])
        wire.close()
    }
    ...
    @Test("alula:close: graceful teardown — ack flushed, then the close frame")
    func gracefulClose() async throws {
        let harness = try Harness()
        let wire = try await harness.wire("/socket?token=alice")
        _ = try await wire.join("room:1")

        try wire.send(ref: "9", topic: "alula", event: "alula:close")
        let ack = try await wire.nextEnvelope()
        #expect(ack == Envelope(ref: "9", topic: "alula", event: "alula:reply", payload: .object([:])))
