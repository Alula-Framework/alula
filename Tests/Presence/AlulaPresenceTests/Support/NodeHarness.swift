...
/// A complete Alula app node for integration tests: container, channels,
...
final class PresenceNode: Sendable {
    ...
    init(
        ...
        guard let presenceService = presenceModule.service as? PresenceService else {
            preconditionFailure("AlulaPresenceModule should contribute exactly one PresenceService")
        }
