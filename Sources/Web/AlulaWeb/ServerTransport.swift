...
/// How a transport reads its settings out of the app-wide Alula
/// configuration (host, port, limits — whatever that transport needs).
/// Letting the *type* do this keeps `AlulaWebModule` generic over
/// transports without knowing any transport's option set.
public protocol ServerTransportConfiguration: Sendable {
    init(configuration: AlulaCore.Configuration) throws
}
...
/// dispatch are **not** behind this seam — they are Alula Web's own, as
...
/// else must work, so anything Alula Web assumes belongs here. Three of
...
///   `Service` contract exactly (§5.3, Alula Core §7).
