...
// MARK: - Route values (what a composition root hands AlulaWebModule)
...
private func userRoutes() -> [RouteRegistration] {
    ...
    return UserController.alulaRoutes(make)
        + EchoSocketController.alulaRoutes { _ in EchoSocketController() }
}
...
struct WebSocketIntegrationTests {
    ...
    func upgradeAtHTTPRouteDoesNotRunHandler() async throws {
        ...
        let dispatch = try TestClient(
            routes: SideEffectController.alulaRoutes { _ in SideEffectController() }
        ).dispatch
...
private func widgetRoutes() -> [RouteRegistration] {
    WidgetController.alulaRoutes { _ in WidgetController() }
        + GadgetController.alulaRoutes { _ in GadgetController() }
}
