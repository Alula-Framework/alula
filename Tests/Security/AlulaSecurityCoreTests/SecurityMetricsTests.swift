...
struct SecurityMetricsTests {
    ...
    func passwordOutcomes() async throws {
        ...
        let events = try await TelemetryTest.capture(prefix: "alula.sign_in") {
    ...
    func tokenOutcomes() async throws {
        ...
        let events = try await TelemetryTest.capture(prefix: "alula.one_time_tokens") {
    ...
    func sessionCounters() async throws {
        ...
        let client = try TestClient(
            ...
            middleware: MiddlewareRegistration.lane(.default, [Sessions(runtime: runtime)])
                + AlulaSecurityModule(validator: nil, sessions: runtime).middleware)
        let events = try await TelemetryTest.capture(prefix: "alula") {
