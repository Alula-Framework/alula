...
/// The seam between Alula's string-native resolution and
...
/// `.string` request outright rather than stringifying it. Alula decodes from
...
struct ProviderInteropTests {
    ...
    @Test("typed scalars in a provider decode through Alula's accessors")
    func typedScalarsDecode() throws {
        let configuration = Configuration(providers: [
            InMemoryProvider(values: [
                "datasource.url": "postgres://localhost:5432/alula",
        ...
        #expect(try configuration.get("datasource.url", as: String.self) == "postgres://localhost:5432/alula")
    ...
    @Test("Alula YAML serves the swift-configuration reader API directly")
    func yamlServesReaderAPI() throws {
        let provider = try AlulaYAMLProvider(
            string: """
            server:
              port: 8080
              host: localhost
...
@Suite("Access reporting from Alula's own accessors")
struct AccessReportingTests {
    ...
    func substitutedValuesAreSecret() throws {
        ...
        setenv("ALULA_TEST_DB_PASSWORD", "hunter2", 1)
        defer { unsetenv("ALULA_TEST_DB_PASSWORD") }

        let provider = try AlulaYAMLProvider(
            string: """
                datasource:
                  url: postgres://localhost/app
                  password: ${ALULA_TEST_DB_PASSWORD}
                """,
            name: "alula.yaml",
...
struct AbsentKeyLookupTests {
    ...
    @Test("Alula's own YAML provider answers absence in one lookup")
    func alulaProviderShortCircuits() throws {
        ...
        let provider = try AlulaYAMLProvider(
            string: "server:\n  port: 8080", name: "alula.yaml", substitution: .none)
        let configuration = Configuration(providers: [provider])
    ...
    func arraysSurviveTheShortcut() throws {
        ...
        let provider = try AlulaYAMLProvider(
            string: "hosts:\n  - alpha\n  - beta", name: "alula.yaml", substitution: .none)
        let reader = ConfigReader(provider: provider)
