...
struct PipelineLaneTests {
    /// The lanes and routes a composition root would hand AlulaWebModule.
    private func client() throws -> TestClient {
        ...
        let routes: [RouteRegistration] =
            [pingRoute]
            + BareLaneController.alulaRoutes { _ in BareLaneController() }
            + AdminController.alulaRoutes { _ in AdminController() }
            + RouteLaneController.alulaRoutes { _ in RouteLaneController() }
