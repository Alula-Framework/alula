...
/// Adapts an Alula `ConfigSource` to a swift-configuration `ConfigProvider`.
///
/// Alula's source protocol is string-native and synchronous: "given a
...
/// API, not just with Alula's accessors.
///
...
struct ConfigSourceProvider: ConfigProvider {
    ...
    func value(forKey key: AbsoluteConfigKey, type: ConfigType) throws -> LookupResult {
        ...
        return LookupResult(
            ...
            value: try ConfigValue(alulaRawString: raw, as: type, key: encodedKey)
        )
    }
...
extension ConfigSourceProvider: AlulaKeyPresenceProvider {
    /// One lookup instead of ten. See ``AlulaKeyPresenceProvider``.
...
extension ConfigValue {
    ...
    init(alulaRawString raw: String, as type: ConfigType, key: String) throws {
        guard let content = ConfigContent.alulaScalar(raw, as: type) else {
            ...
            // `AlulaYAMLSnapshot`'s job, where the structure is known.
            throw ConfigError.decodingFailed(
                key: key, rawValue: raw, targetType: type.alulaTargetTypeName)
        }
