...
struct ConfigPrefixTests {
    ...
    @Test("the default reproduces every spelling Alula shipped before the type")
    func defaultMatchesHistoricalSpellings() {
        let prefix = ConfigPrefix.default
        #expect(prefix.baseFileName == "alula.yaml")
        #expect(prefix.environmentFileName(for: .prod) == "alula-prod.yaml")
        #expect(prefix.environmentVariable == "ALULA_ENV")
        #expect(prefix.variableName(for: "datasource.url") == "ALULA_DATASOURCE_URL")
        #expect(prefix.variableName(for: "datasource.pool_size") == "ALULA_DATASOURCE_POOL_SIZE")
    }
    ...
    @Test("AlulaConfigFiles still agrees with the default prefix by construction")
    func filesAgreeWithDefault() {
        #expect(AlulaConfigFiles.base == ConfigPrefix.default.baseFileName)
        #expect(
            AlulaConfigFiles.environmentFile(for: .test)
...
struct ConfigurationLoadPrefixTests {
    ...
    private func withConfigDirectory<T>(
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("alula-prefix-tests-\(UUID().uuidString)")
    ...
    @Test("alula.yaml is not consulted under a custom prefix")
    func defaultNameIsNotAFallback() throws {
        // Falling back to alula.yaml would make the prefix advisory, and an
    ...
    @Test("<PREFIX>_ENV selects the overlay, and ALULA_ENV no longer does")
    ...
    @Test("the env-var layer reads <PREFIX>_ names and ignores ALULA_ ones")
    ...
    func missingKeyNamesThePrefixedVariable() throws {
        try withConfigDirectory(files: ["myapp.yaml": baseYAML]) { directory in
            ...
            do {
                ...
                #expect(!message.contains("ALULA_"))
            }
        }
    }
