...
///     FLIGHT_TEST_KEYCLOAK_URL=http://localhost:8089 swift test --enable-all-traits --filter KeycloakSignIn
private let keycloakURL = ProcessInfo.processInfo.environment["FLIGHT_TEST_KEYCLOAK_URL"]

@Suite(
    "OIDC sign-in against a real Keycloak",
    .enabled(if: keycloakURL != nil, "set FLIGHT_TEST_KEYCLOAK_URL; see CI/keycloak/start.sh"))
