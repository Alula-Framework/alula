...
/// `AlulaPubSubModule` as a dependency, and remember to expose
/// `PubSubRelayService` as the module's service — and forgetting the third
/// gave a cluster that relayed nothing, silently. `AlulaPubSubModule` takes
...
private struct InMemoryAdapterModule: AlulaModule {
...
struct ModuleTests {
    ...
    func withAdapter() async throws {
        ...
        let pubsub = try AlulaPubSubModule(
        ...
        #expect(app.services[0].moduleName == "AlulaPubSubModule")
        #expect(app.moduleOrder == ["InMemoryAdapterModule", "AlulaPubSubModule"])
    ...
    func endToEndTwoApps() async throws {
        ...
        func app() throws -> (app: AssembledApplication, bus: any PubSub) {
            ...
            let pubsub = try AlulaPubSubModule(
                configuration: configuration, adapter: adapterModule.adapter)
            let assembled = try Alula.assemble(
                configuration: configuration, modules: [adapterModule, pubsub])
    ...
    @Test("AlulaPubSubModule declares no dependencies, and has no service without an adapter")
    func moduleShape() throws {
        #expect(AlulaPubSubModule.dependencies.isEmpty)
        #expect(try AlulaPubSubModule(configuration: Configuration()).service == nil)
    }
}
...
struct UnloadedAdapterTests {
    ...
    func configuredButNotLoaded() throws {
        ...
        let error = #expect(throws: (any Error).self) {
            _ = try AlulaPubSubModule(configuration: configuration)
        }
        ...
        // being true when alula-data grew AlulaPubSubValkeyModule — so the
        // error sent an operator to build something they already had.
        #expect(message.contains("AlulaPubSubValkeyModule"))
    }
    ...
    func messageIsActionable() {
        let error = UnloadedAdapterError(
            ...
            module: "AlulaPubSubValkeyModule")

        // Whoever hits this at 3am gets both ways out of it, by name.
        #expect(error.description.contains("AlulaPubSubValkeyModule"))
        #expect(error.description.contains("pubsub.valkey.url"))
        #expect(error.description.contains("Alula.bootstrap(modules:)"))
    }
