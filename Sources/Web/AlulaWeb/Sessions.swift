...
/// request, provided by ``AlulaSessionsModule``.
///
...
/// and stop (D27). Same reason `AlulaCacheModule` provides a `CacheRuntime`
...
public final class SessionRuntime: Sendable {
    ...
    ///     ``AlulaSessionsModule/init(configuration:store:)``.
...
/// once the handler has returned. ``AlulaSessionsModule`` puts it in the
/// default lane.
///
...
public struct Sessions: Middleware {
    ...
    public func handle(_ context: RequestContext, next: Next) async throws -> Response {
        ...
        // in a lane — `AlulaSessionsModule` in the default lane for every
        // application, and `AlulaSecurityModule` ahead of `Authentication`
...
/// its business to handle. `Authentication` in AlulaSecurityCore conforms.
...
public struct SessionNotConfiguredError: Error, Sendable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        AlulaSessionsModule in the application's modules, and if the route names its own \
