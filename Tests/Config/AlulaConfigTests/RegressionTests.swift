...
struct ParsingRobustnessTests {
    ...
    func bomIsStripped() throws {
        let document = try AlulaYAMLDocument(
    ...
    func unreadableFileMessageIsHumane() throws {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("alula-audit-\(UUID().uuidString)")
            .appendingPathComponent("alula.yaml")
        ...
        do {
            _ = try AlulaYAMLDocument(contentsOf: directory)
...
struct ErrorErgonomicsTests {
    ...
    func localizedDescriptionIsUseful() {
        ...
        let load = ConfigLoadError.missingBaseFile(expectedPath: "/etc/alula.yaml")
        #expect(load.localizedDescription.contains("/etc/alula.yaml"))
    }
