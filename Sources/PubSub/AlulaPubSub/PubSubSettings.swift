...
/// Per-subscriber buffering, as a deployment writes it in `alula.yaml`.
///
/// A separate type from `LocalPubSub.BufferingPolicy` (which is
/// `AsyncStream.Continuation.BufferingPolicy`, a stdlib type) because
/// conforming that retroactively would attach Alula's configuration format
...
/// The PubSub module's settings, read from `alula.yaml` once when the module
/// is built. They are deployment knobs, so they live where deployment knobs
/// live: `AlulaPubSubModule.init(configuration:)` reads them, and the module
