...
/// `alula:presence_state` and any number of `alula:presence_diff`
...
public struct PresenceSync: Sendable, Equatable {
    ...
    /// Replaces the whole view with a `alula:presence_state` payload.
    ...
    /// Applies a `alula:presence_diff` payload.
    @discardableResult
