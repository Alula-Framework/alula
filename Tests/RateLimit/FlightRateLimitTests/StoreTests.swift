...
struct QuotaAndModuleTests {
    ...
    func defaults() throws {
        ...
        let bounded = try FlightRateLimitModule(
    ...
    func adapter() throws {
        let module = try FlightRateLimitModule(
    ...
    func unloadedAdapter() throws {
        ...
        #expect(throws: UnloadedAdapterError.self) {
            try FlightRateLimitModule(configuration: configuration)
        }
        do {
            _ = try FlightRateLimitModule(configuration: configuration)
        } catch let error as UnloadedAdapterError {
            #expect(error.module == "FlightRateLimitValkeyModule")
