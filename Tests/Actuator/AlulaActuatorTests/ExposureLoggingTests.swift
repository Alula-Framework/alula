...
struct ExposureLoggingTests {
    ...
    func fullOutsideDevelopmentWarns() {
        ...
        #expect(text.contains("ALULA_ACTUATOR_EXPOSURE"))
    }
    ...
    func healthOnlyIsAnnounced() {
        let capture = Capture()
        _ = ActuatorModule(
            environment: AlulaEnvironment("production"), exposure: .healthOnly,
            logger: capture.logger)
