...
///     AlulaPasswordSignInModule.self,   // ← or AlulaOIDCSignInModule.self
///     AppModule.self,                    // provides `credentialStore: any CredentialStore`
/// ]
/// ```
///
/// Provides `signInProvider: any SignInProvider` — the same type
/// ``AlulaOIDCSignInModule`` provides, so a controller written against it
...
///
/// Takes the application's `any CredentialStore` and `AlulaRateLimitModule`'s
/// `RateLimiter`, matched by type. Reads `security.password.issuer` for the
/// principals it produces; `local` when unset.
public final class AlulaPasswordSignInModule: AlulaModule {
    public static var dependencies: [any AlulaModule.Type] {
        [AlulaSecurityModule.self, AlulaSessionsModule.self, AlulaRateLimitModule.self]
    }
...
/// ``AlulaPasswordSignInModule``. It does not also validate bearer tokens:
/// list ``AlulaOIDCModule`` beside it for an API that accepts them, and the
/// two share one `security.oidc` block.
public final class AlulaOIDCSignInModule: AlulaModule {
    public static var dependencies: [any AlulaModule.Type] {
