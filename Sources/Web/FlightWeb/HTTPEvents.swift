...
///
/// ``FlightWebModule`` contributes ``HTTPMetrics/definitions``, so an
/// application with `FlightTelemetryModule` reports request counts and
...
public enum HTTPEvents {
    ...
    /// body is written.
    @TelemetryEvent("flight.http.request")
...
public enum HTTPMetrics {
    ...
    /// `status`.
    public static let requests = "flight_http_requests"
    /// ``HTTPEvents/RequestHandled``'s duration, by `method` and `route`.
    public static let duration = "flight_http_request_duration"

    /// What ``FlightWebModule`` contributes to the reported metrics.
    public static let definitions: [TelemetryMetric] = [
        .counter(
            HTTPEvents.RequestHandled.self, name: "flight.http.requests",
            tags: \.method, \.route, \.status),
        .distribution(
            HTTPEvents.RequestHandled.self, \.duration, name: "flight.http.request.duration",
