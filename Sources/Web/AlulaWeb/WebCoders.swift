...
/// `{"status": …, "error": …}` object for failures. That made Alula
...
///
/// Built by ``AlulaWebModule`` from `alula.yaml`, and reachable from any
/// handler as `context.coders`. Provide your own to override — a module holds
/// it as a value the composition root hands ``AlulaWebModule`` (matched by
/// type, as its `coders:` parameter):
///
/// ```swift
/// var coders = WebCoders.default
/// coders.jsonEncoder.keyEncodingStrategy = .convertToSnakeCase
/// // expose `coders` from a module; the composer wires it to AlulaWebModule
/// ```
public struct WebCoders: Sendable {
    ...
    /// RFC 9457 names — can say so without Alula enumerating the options in
    /// advance.
...
/// body, and what Alula answers with unless told otherwise.
///
/// ```json
...
extension WebCoders {
    ...
    /// ```
    public init(configuration: AlulaCore.Configuration) throws {
        var coders = WebCoders.default
...
/// The pre-RFC-9457 shape Alula used to hardcode, kept for applications
...
/// A `web.*` key whose value Alula does not recognize. Raised at startup,
