#!/usr/bin/env bash
#
# Fails if a consumer depending on Flight the ordinary way — a version
# requirement, every trait on — cannot resolve.
#
# check-lean-consumer.sh cannot catch this class of failure, and did not:
# its consumer takes Flight by *path*, and SwiftPM exempts path dependencies
...
# `branch:`. Every real application takes Flight by version, so the only
...
mkdir -p "$scratch/flight"
(cd "$root" && git -c safe.directory='*' ls-files -co --exclude-standard -z) \
  | (cd "$root" && xargs -0 cp --parents -t "$scratch/flight")
(
  cd "$scratch/flight"
  git init -q
  git add -A
...
if ! (cd "$scratch/consumer" && swift package resolve) >"$scratch/resolve.log" 2>&1; then
  ...
  echo "::error::a consumer depending on Flight by version, with every trait on, cannot resolve."
