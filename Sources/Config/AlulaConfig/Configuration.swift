...
/// to `Alula.bootstrap`, never mutated after.
///
...
/// `alula-{env}.yaml` over `alula.yaml`) a key-by-key merge rather than a
...
/// API directly — where a bad value is a thrown error — and applies Alula's
/// own `ConfigDecodable` on top.
///
/// Reach for `reader` when handing config to a library that speaks
/// swift-configuration; reach for `get`/`getIfPresent` for Alula's own code.
///
...
public struct Configuration: Sendable {
    ...
    /// — under a custom prefix, telling them to set `ALULA_SERVER_PORT` would
    /// be wrong.
    ...
    ///     `[env vars, alula-{env}.yaml, alula.yaml]`.
    ///   - environment: The resolved `AlulaEnvironment`, if this stack was
    ...
    ///     environment-variable namespace. Defaults to `alula`. Carried on the
    ...
    ///
    /// Alula's own source protocol predates the move onto
    ...
    ///   (`ALULA_SERVER_PORT=eighty`) reaches this path and aborts the
    ...
    /// is the normal path; this exists for tooling and diagnostics (Alula
            ...
            reporter.report(
                AccessEvent(
                    metadata: .init(
                        accessKind: .get,
                        key: absoluteKey,
                        // Alula resolves raw strings and decodes them itself,
    ...
    /// request outright rather than stringifying it. Alula decodes from raw
    /// strings, so asking only for `.string` would silently lose every
    /// non-string value in any file-backed or third-party provider.
    ///
    /// So: ask for `.string` first (the common case, and the only one Alula's
    ...
    private static func lookup(
        ...
        // Alula's own providers know their key set outright, so a miss costs
        ...
        // correct — just slower.
        if let presence = provider as? any AlulaKeyPresenceProvider,
            !presence.alulaHoldsKey(key)
        {
            return .absent
        }

        // Scalars first: the common case, and the only shape Alula's own
        ...
        for type in [ConfigType.string, .int, .double, .bool] {
            do {
                ...
                if let raw = value.content.alulaRawString {
        ...
        for type in nonScalarTypes {
            do {
                ...
                return .unrepresentable(type.alulaKindDescription)
            } catch {
                if refusal == nil { refusal = error }
            }
        }
    ...
    /// Hummingbird, and the Swift Temporal SDK all do — so an Alula app
    /// configures them from the same layered stack that configures Alula,
    ...
    /// why Alula's own code should not resolve config through this property.
...
extension ConfigContent {
    ...
    /// The raw-string rendering Alula's `ConfigDecodable` decodes from.
    ///
    ...
    /// type directly.
    fileprivate var alulaRawString: String? {
        switch self {
...
extension ConfigType {
    ...
    fileprivate var alulaKindDescription: String {
        switch self {
