...
///
/// `InMemorySessionStore` ships here and is the default. `AlulaSessionsValkey`
/// in alula-data implements this over Valkey for deployments with more than
/// one replica, and `AlulaSessionsTesting`'s `RecordingSessionStore` is the
/// one for tests.
