...
/// an unset `ALULA_ENV` resolves to `dev`, and any spelling the code does
...
struct GatingTests {
    ...
    @Test("development environments serve the dashboard",
          arguments: [AlulaEnvironment.dev, .test, AlulaEnvironment("local")])
    func developmentServesDashboard(environment: AlulaEnvironment) async throws {
    ...
    @Test("anything not recognized as development gets health only",
          arguments: [
            AlulaEnvironment.staging, .prod,
            AlulaEnvironment("production"), AlulaEnvironment("PROD"),
            AlulaEnvironment("prd"), AlulaEnvironment("live"),
          ])
    func unrecognizedEnvironmentsAreClosed(environment: AlulaEnvironment) async throws {
    ...
    @Test("an unrecognized ALULA_ACTUATOR_EXPOSURE stops startup")
    func unknownExposureRejected() {
        #expect(throws: ActuatorConfigurationError.self) {
            _ = try ActuatorExposure.resolve(
                environment: .prod,
                processEnvironment: ["ALULA_ACTUATOR_EXPOSURE": "everything"])
        }
    }
        ...
        for name in ["staging", "prod", "production", "PROD", "prd", "qa", "anything"] {
            #expect(
                try ActuatorExposure.resolve(
                    environment: AlulaEnvironment(name), processEnvironment: [:]) == .healthOnly,
    ...
    func undeclaredEnvironmentGetsHealthOnly() async throws {
        ...
        // ALULA_ENV resolves to `dev`, `dev` is on the allowlist, so a
    ...
    func healthOnlyPublishesEveryProbe() {
        ...
        let actuator = ActuatorModule(environment: AlulaEnvironment("production"))
        #expect(
