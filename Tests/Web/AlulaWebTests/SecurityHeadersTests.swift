...
struct SecurityHeadersTests {
    ...
    private func client(_ headers: SecurityHeaders = .default) throws -> TestClient {
        try TestClient(
            routes: SecurityHeadersController.alulaRoutes { _ in SecurityHeadersController() },
    ...
    @Test("AlulaWebModule reads the policy from configuration")
    func moduleReadsConfiguration() async throws {
        let module = try AlulaWebModule<InMemoryTransport>(
            configuration: Configuration(values: [
                SecurityHeadersConfigKey.frameOptions: "sameorigin"
            ]),
            routes: SecurityHeadersController.alulaRoutes { _ in SecurityHeadersController() },
