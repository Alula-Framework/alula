...
struct ChannelsFixtureModule: AlulaModule {
    /// Declared as values, so this module no longer depends on
    /// `AlulaChannelsModule` — it is the other way round now.
    ...
    func socketRoutes(_ channels: AlulaChannelsModule) -> [RouteRegistration] {
        [
...
struct Harness {
    ...
    let channels: AlulaChannelsModule
    /// The fixture's own probe, held rather than resolved.
    let events: ChannelEvents
    private let pubsub: AlulaPubSubModule
...
extension ChannelWireClient {
    ...
    func join(_ topic: String, ref: String = "j1") async throws -> Envelope? {
        try send(ref: ref, topic: topic, event: "alula:join")
