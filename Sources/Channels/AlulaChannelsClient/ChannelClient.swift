...
public struct ChannelMessage: Sendable, Equatable {
    ...
    /// `"alula:join"` and `payload` the rejoin's initial state. Application
    /// events can never collide with it — the server refuses `alula:`-
...
/// `AlulaChannelsProtocol` and swift-log behind it.
///
...
public actor ChannelClient {
    ...
    public init(
        ...
        logger: Logger = Logger(label: "alula.channels.client")
    ) {
        self.url = url
    ...
    /// Graceful teardown: best-effort `alula:close`, transport
    ...
    /// channel's message stream as a `alula:join` message carrying the
    /// fresh initial state; a rejected rejoin surfaces as `alula:error`
