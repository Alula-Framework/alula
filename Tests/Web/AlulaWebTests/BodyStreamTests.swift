...
// transport actually not buffering — lives in AlulaTransportTests.
...
/// routes a composition root would hand `AlulaWebModule`. `/mismatched` is a
/// buffered-delivery route whose handler asks for a stream, which must fail
/// loudly rather than hang.
private func streamRoutes() -> [RouteRegistration] {
    StreamFixtureController.alulaRoutes { _ in StreamFixtureController() }
        + [
            ...
            RouteRegistration(method: "POST", path: "/mismatched", source: "StreamModule") {
                context in
                _ = try AlulaWeb.decodeRequestBody(RequestBodyStream.self, from: context)
