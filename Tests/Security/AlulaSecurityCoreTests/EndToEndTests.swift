...
/// path, no network) — standing in for Alula Security OIDC configuring the
/// generic validator. A plain value holder: the validator it builds is handed
/// to `AlulaSecurityModule(validator:)`.
private struct InMemoryOIDCModule {
    /// What `AlulaSecurityModule` takes.
...
/// the values a composition root hands `AlulaWebModule`.
...
@Suite("End to end through Alula Web's real pipeline")
