...
/// What sign-in reports, as telemetry events — Alula's own providers, and
/// any provider of yours that emits them too, which is what puts a custom
/// sign-in on the same dashboards as the built-in ones.
///
/// ``AlulaSecurityModule`` contributes ``SignInMetrics/definitions``, so an
...
public enum SignInEvents {
    ...
    @TelemetryEvent("alula.sign_in.started")
    public enum Started {
    ...
    @TelemetryEvent("alula.sign_in.attempt")
    public enum Attempt {
    ...
    @TelemetryEvent("alula.sign_in.password_rehashed")
    public enum PasswordRehashed {}

    /// A session sign-in that reached `sessions.authenticated-lifetime` and
    /// was signed out.
    @TelemetryEvent("alula.sign_in.expired")
    public enum Expired {}

    /// A one-time token issued.
    @TelemetryEvent("alula.one_time_tokens.issued")
    ...
    @TelemetryEvent("alula.one_time_tokens.redemption")
...
public enum SignInMetrics {
    ...
    public static let started = "alula_sign_in_started"
    /// ``SignInEvents/Attempt``, counted by `provider` and `outcome`.
    public static let attempts = "alula_sign_in_attempts"
    /// ``SignInEvents/Attempt``'s duration, by `provider`.
    public static let duration = "alula_sign_in_duration"
    /// ``SignInEvents/PasswordRehashed``, counted.
    public static let passwordRehashes = "alula_sign_in_password_rehashes"
    /// ``SignInEvents/Expired``, counted.
    public static let expired = "alula_sign_in_expired"
    /// ``SignInEvents/TokenIssued``, counted by `purpose`.
    public static let tokensIssued = "alula_one_time_tokens_issued"
    /// ``SignInEvents/TokenRedemption``, counted by `purpose` and `outcome`.
    public static let tokensRedeemed = "alula_one_time_tokens_redeemed"

    /// What ``AlulaSecurityModule`` contributes to the reported metrics.
    public static let definitions: [TelemetryMetric] = [
        .counter(SignInEvents.Started.self, tags: \.provider),
        .counter(
            SignInEvents.Attempt.self, name: "alula.sign_in.attempts", tags: \.provider, \.outcome),
        .distribution(
            SignInEvents.Attempt.self, \.duration, name: "alula.sign_in.duration",
            unit: .milliseconds,
            tags: \.provider),
        .counter(SignInEvents.PasswordRehashed.self, name: "alula.sign_in.password_rehashes"),
        .counter(SignInEvents.Expired.self),
        .counter(SignInEvents.TokenIssued.self, tags: \.purpose),
        .counter(
            SignInEvents.TokenRedemption.self, name: "alula.one_time_tokens.redeemed",
