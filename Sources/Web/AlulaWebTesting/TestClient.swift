...
/// Drives the full Alula Web pipeline — user middleware, routing, encoding,
...
public struct TestClient: Sendable {
    ...
    /// composition root hands `AlulaWebModule`, plus the `WebRuntime` that
    ...
    public init(
        ...
        var logger = Logger(label: "alula.web.test-client")
...
extension Response {
    ...
    ///
    /// Prefer ``AlulaWebTesting/TestClient/decodeJSON(_:from:)`` when the
