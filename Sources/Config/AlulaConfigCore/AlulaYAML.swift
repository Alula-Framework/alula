/// The Alula YAML subset parser.
///
/// Alula Config deliberately parses a *subset* of YAML rather than depending
    ...
    static func parse(_ text: String) throws -> Node? {
        ...
        guard case .mapping = root else {
            throw ParseError(
                ...
                message: "the top level of an Alula config file must be a mapping of keys, not a sequence or scalar"
            )
        }
        return root
    }
    ...
    private static func makeLines(_ text: String) throws -> [Line] {
        ...
        for rawLine in text.split(omittingEmptySubsequences: false, whereSeparator: { lineBreaks.contains($0) }) {
            ...
            if content.hasPrefix("%") {
                throw ParseError(
                    line: number, column: indent + 1,
                    message: "YAML directives ('%…') are not supported by the Alula YAML subset"
                )
            }
            if content == "..." {
            ...
            if content == "---" || content.hasPrefix("--- ") {
                ...
                throw ParseError(
                    line: number, column: indent + 1,
                    message: "multiple YAML documents in one file are not supported — one alula config file is one document"
                )
            }
    ...
    private struct Parser {
            ...
            while let line = peek() {
                ...
                if line.indent > indent {
                    throw ParseError(
                        ...
                        message: "unexpected indentation — expected a key at indentation \(indent) (multi-line plain scalars are not supported by the Alula YAML subset; quote long values)"
                    )
                }
        ...
        private mutating func parseMappingEntry(
            ...
            if key == "<<" {
                throw ParseError(
                    ...
                    message: "merge keys ('<<:') are not supported by the Alula YAML subset — repeat the keys explicitly"
                )
            }
            ...
            if content.hasPrefix("\"") || content.hasPrefix("'") {
                ...
                guard let colon = keyColonIndex(in: content) else {
                    throw ParseError(
                        line: line.number,
                        column: line.indent + 1,
                        message: "expected 'key: value', 'key:', or '- item' — found a bare scalar (multi-line plain scalars are not supported by the Alula YAML subset)"
                    )
                }
                ...
                if let bad = unsupportedLeadingMarker(rawKey) {
                    throw ParseError(
                        line: line.number, column: line.indent + 1,
                        message: "\(bad) is not supported by the Alula YAML subset — quote the key if the character is literal"
                    )
                }
                key = rawKey
        ...
        private func parseScalar(text: String, line: Line) throws -> Node {
            ...
            if let bad = unsupportedLeadingMarker(text) {
                throw ParseError(
                    line: line.number, column: line.indent + 1,
                    message: "\(bad) is not supported by the Alula YAML subset — quote the value if the character is literal"
                )
            }
        ...
        private func parseQuoted(_ text: Substring, line: Line) throws -> (value: String, rest: Substring) {
            ...
            throw ParseError(
                line: line.number, column: line.indent + 1,
                message: "unterminated \(quote == "'" ? "single" : "double")-quoted scalar (multi-line scalars are not supported by the Alula YAML subset)"
            )
        }
