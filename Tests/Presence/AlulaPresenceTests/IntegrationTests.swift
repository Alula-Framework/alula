...
struct IntegrationTests {
    ...
    @Test("on join: reply first, then alula:presence_state with the joiner included")
    ...
    @Test("client alula:leave untracks that topic only")
    func clientLeave() async throws {
        ...
        try bob.send(ref: "l1", topic: "room:1", event: "alula:leave")
        let diff = try await alice.next(event: PresenceEvent.diff)
        let leaves = PresenceWire.entries(from: diff.payload["leaves"] ?? .null)
...
struct ModuleTests {
    ...
    func replicaIdentity() async throws {
        ...
        #expect(AlulaPresenceModule.generateBoot() != AlulaPresenceModule.generateBoot())
    }
}
