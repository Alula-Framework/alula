...
/// 1. `init(_alulaConfiguration:)` — one line per stored property, binding it
...
///
/// The exact expansions are pinned by `Tests/Core/AlulaCoreMacroTests/SettingsMacroFixtureTests`.
public struct SettingsMacro: MemberMacro, ExtensionMacro {
    ...
    public static func expansion(
        ...
        // (`_alulaConfiguration` — the label the graph passes). Validation,
        // which used to run in the registration factory at freeze, runs here:
        // a bad value fails composition. The container-era init(_alula:) and
        ...
        let settingsInit: DeclSyntax = """
            \(raw: access)init(_alulaConfiguration configuration: AlulaCore.Configuration) throws {\(raw: initBody)}
            """
    ...
    // camelCase -> kebab-case lives in AlulaConfigCore's ConfigKeyNaming —
    // shared with alula-registration-gen, which needs the identical
    // derivation for its build-time alula.yaml check. One implementation,
