...
/// Inter-node presence traffic: rides Alula PubSub on one
...
///
/// One topic, not a `alula:presence:<topic>`
/// family: PubSub is exact-match with no wildcards (PubSub), so a node
...
public enum PresenceGossip {
    ...
    public static let topic = "alula:presence"
...
/// message payload (opaque to PubSub, PubSub).
