...
/// `ALULA_ENV` resolves to `dev`, and any spelling the code does not
...
public enum ActuatorExposure: String, Sendable, CaseIterable {
    ...
    ///
    /// Read from the process environment rather than `alula.yaml`, for the
    /// same reason `ALULA_ENV` is: this decides whether a route is
    /// *registered at all*, and registration happens before configuration is
    /// resolvable. `ALULA_ACTUATOR_EXPOSURE` is the env-var spelling of
    /// `actuator.exposure` under Alula Config's own naming convention, so it
    ...
    ///     `ALULA_ENV` set, or an embedder naming it in code — as opposed to
    ///     defaulted. See the discussion below; a default is not a
    ///     declaration.
    ///   - processEnvironment: Where the `ALULA_ACTUATOR_EXPOSURE` override
    ///     is read from.
    public static func resolve(
        ...
        if let raw = processEnvironment["ALULA_ACTUATOR_EXPOSURE"], !raw.isEmpty {
        ...
        // An unset `ALULA_ENV` resolves to `dev`, and `dev` is in the
