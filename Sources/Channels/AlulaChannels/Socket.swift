...
public final class Socket: Sendable, Identifiable {
    ...
    /// The connection's logger (request metadata already stamped by Alula
    /// Web's dispatch).
    ...
    public func push(topic: String, event: String, payload: JSONValue = .object([:])) {
        ...
        //
        // The framework filters `alula:`-prefixed events arriving from a
        ...
        // pattern, and a client sending `{"type": "alula:x"}` would have
        ...
        guard !event.hasPrefix(ReservedEvent.prefix) else {
            logger.error(
                "refusing to push an event in the reserved alula: namespace",
    ...
    // The three members below exist for Alula's own packages layered on
    // Channels (Presence today, Live later) and are deliberately hidden
    // behind `@_spi(AlulaInternal)`: application code sees none of this
    // unless it opts in with an SPI import, which keeps the reserved
    // `alula:` namespace and the membership lifecycle out of casual reach
    ...
    /// Pushes a *reserved* (`alula:`-namespaced) event to this socket —
