...
/// Serves one already-parsed `AlulaYAMLSnapshot`.
///
/// `FileProvider<AlulaYAMLSnapshot>` is the general path and reads the file
/// itself, but its initializer is `async` — and Alula parses its layers
...
public struct AlulaYAMLProvider: ConfigProvider {

    /// The document this provider serves.
    public let yamlSnapshot: AlulaYAMLSnapshot

    public init(snapshot: AlulaYAMLSnapshot) {
    ...
    public init(
        ...
        self.yamlSnapshot = AlulaYAMLSnapshot(
            document: try AlulaYAMLDocument(
...
extension AlulaYAMLProvider: CustomStringConvertible, CustomDebugStringConvertible {
    ...
    ///
    /// ```swift
    /// // alula.yaml:  db: { host: localhost, password: "${DB_PASSWORD}" }
    ...
    public var debugDescription: String {
        ...
        let values = document.keys
            ...
            .joined(separator: ", ")
        return "AlulaYAML[\(providerName), \(document.keys.count) keys: \(values)]"
    }
}

extension AlulaYAMLProvider: AlulaKeyPresenceProvider {
    ...
    /// exactly when the scalar probe does. See ``AlulaKeyPresenceProvider``.
    func alulaHoldsKey(_ key: AbsoluteConfigKey) -> Bool {
