...
public enum Stereotype: Sendable, Equatable, CaseIterable {
    ...
    case repository
    /// HTTP entry points. The `@Controller` *macro* lives in Alula Web (it
    ...
    /// its own answer.
    case settings
    /// A `@Middleware` type. The `@Middleware` *macro* lives in Alula Web,
...
public struct ComponentDescriptor: Sendable, Equatable {
    ...
    /// Which AlulaModule registered this (stamped by bootstrap around each
    ...
    /// build knows what it scanned, and Actuator renders that — there is no
