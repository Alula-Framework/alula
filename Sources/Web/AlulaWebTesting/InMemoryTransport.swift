...
/// bootstrap path — module DAG, composition, `AlulaWebModule`, dispatch assembly,
...
///         modules: try alulaComposeModules(config, ModuleHealthRegistry()))
///     // from another task:
...
/// inside `AlulaWebModule`'s service and tests need a handle to it. One
...
public enum InMemoryTransportHub {
    ...
    public enum HubError: Error, CustomStringConvertible {
        ...
        public var description: String {
            "No InMemoryTransport is running — bootstrap with AlulaWebModule<InMemoryTransport> first (and await readiness)."
        }
    }
