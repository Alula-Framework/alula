...
///
/// Alula Web never interprets this — it stores it, and authentication
/// middleware writes it. The seam is a protocol rather than a concrete type
/// because `RequestContext` lives here and `AlulaSecurityCore.Principal`
...
///
/// Exactly the shape `AlulaChannels` already uses for the same problem —
...
/// implementation. `AlulaSecurityCore.Principal` conforms to both.
