...
func wireRoutes() -> [RouteRegistration] {
    var routes: [RouteRegistration] = WireController.alulaRoutes { _ in WireController() }
...
/// Boots a `AlulaTransport` on an ephemeral port with the fixture app,
...
func withRunningServer(
    ...
    tls: AlulaTransportConfiguration.TLS? = nil,
    ...
    let transport = AlulaTransport(configuration: configuration, dispatch: dispatch)
