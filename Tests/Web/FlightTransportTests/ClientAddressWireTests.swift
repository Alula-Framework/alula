...
/// `FlightTransport`, over an actual socket. Everything about
/// `TrustedProxies`' own logic is proven without a socket in
/// `TrustedProxiesTests`; this proves the wire is plugged in at all.
@Suite("FlightTransport client address wire behavior", .serialized)
struct ClientAddressWireTests {
    private let routes = ClientAddressWireController.flightRoutes { _ in
