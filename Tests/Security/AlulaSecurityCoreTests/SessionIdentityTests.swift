...
struct SessionIdentityTests {
    ...
    func endToEnd() async throws {
        ...
        let sessionsModule = try AlulaSessionsModule(
        ...
        let client = try TestClient(
            ...
            middleware: sessionsModule.middleware
                + AlulaSecurityModule(validator: validator, sessions: sessionsModule.runtime)
                .middleware)
    ...
    private func revocationClient(_ store: any SessionStore) throws -> TestClient {
        ...
        return try TestClient(
            routes: routes,
            middleware: sessionsModule.middleware
                + AlulaSecurityModule(validator: validator, sessions: runtime).middleware)
    }
