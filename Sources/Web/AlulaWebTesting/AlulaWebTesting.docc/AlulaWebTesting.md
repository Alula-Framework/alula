...
Testing an Alula application at three sizes, none of which need a port.
...
let client = try TestClient(routes: [
    OrderController._alulaRoute_show_0 { _ in OrderController(orders: StubOrderService()) }
])
...
to `ServerTransport`, so `AlulaWebModule` boots against it and every layer
