...
///     AlulaRateLimitValkeyModule.self,   // alula-data; omit for one replica
///     AppModule.self,
...
/// `RateLimiting` middleware in `AlulaWeb` is one consumer; a login
...
public struct AlulaRateLimitModule: AlulaModule {
            ...
            try configuration.requireNoUnloadedAdapter(
                ...
                candidates: [
                    AdapterCandidate(
                        ...
                        module: "AlulaRateLimitValkeyModule")
                ])
        }
        let maxEntries =
...
///
/// AlulaRateLimit cannot import AlulaRateLimitValkey — it lives in
/// alula-data, and the dependency runs the other way. A string constant is
