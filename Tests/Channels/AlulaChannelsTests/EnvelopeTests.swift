...
struct EnvelopeTests {
    ...
    @Test("reserved events are exactly the alula: set")
    func reservedEvents() {
        #expect(ReservedEvent.allCases.map(\.rawValue).sorted() == [
            "alula:close", "alula:error", "alula:heartbeat",
            "alula:join", "alula:leave", "alula:reply",
        ])
        #expect(ReservedEvent(rawValue: "new_msg") == nil)
        #expect(ReservedEvent(rawValue: "alula:join") == .join)
    }
}
