...
/// be proven statically (because it depends on `ALULA_ENV` or an environment
...
public enum ConfigError: Error, CustomStringConvertible, Sendable, Equatable, Hashable {
    ...
    case missingKey(key: String, environment: AlulaEnvironment?, prefix: ConfigPrefix = .default)
    ...
    ///
    /// Read these through `Configuration.reader` (in AlulaConfig) and ask for the array
    /// type directly.
...
public enum ConfigLoadError: Error, CustomStringConvertible, Sendable, Equatable, Hashable {
    /// `alula.yaml` — the base layer, expected in every environment — was not found. Only the environment-specific
    /// `alula-{env}.yaml` is optional; an app configured purely by
    ...
    /// The file is not valid Alula-subset YAML. The message names the
    ...
    /// Configuration written for the framework's name before it became Alula
    /// was found: `FLIGHT_ENV` without `ALULA_ENV`, or a `flight.yaml` or
    /// `flight-{env}.yaml` beside the configuration.
    ///
    /// Reading on would be quietly wrong, not merely incomplete. An unset
    /// `ALULA_ENV` resolves to `dev`, so a production deployment still
    /// setting `FLIGHT_ENV=prod` would start as `dev` — dev overlay, dev
    /// actuator exposure — and a `flight-prod.yaml` beside a renamed base
    /// file would never be layered in (D45).
    case preRenameConfiguration(variables: [String], files: [String])
    ...
    public var description: String {
        ...
        case .unresolvedSubstitution(let file, let line, let key, let variable):
            ...
            """
        case .preRenameConfiguration(let variables, let files):
            let found = (variables + files).joined(separator: ", ")
            return """
            Found configuration from before Flight was renamed Alula: \(found). \
            Alula reads ALULA_* variables and alula.yaml / alula-{env}.yaml, and \
            would otherwise ignore these — an unset ALULA_ENV means dev. Rename \
            each (FLIGHT_ → ALULA_, flight → alula), or keep the old names by \
            passing prefix: ConfigPrefix("flight") to Configuration.load.
            """
        }
    }
}
