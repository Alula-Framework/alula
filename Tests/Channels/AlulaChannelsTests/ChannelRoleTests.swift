...
struct ChannelRoleTests {
    ...
    func rolesGateTheJoin() async throws {
        ...
        try wire.send(ref: "1", topic: "admin:settings", event: "alula:join")
    ...
    func rolesAreAnyOf() async throws {
        ...
        try viaAuditor.send(ref: "1", topic: "reports:q3", event: "alula:join")
        #expect(try await viaAuditor.nextEnvelope()?.event == "alula:reply")
        viaAuditor.close()

        let viaNeither = try await wire(builds, as: "guest")
        try viaNeither.send(ref: "1", topic: "reports:q3", event: "alula:join")
    ...
    func noRolesMeansOpen() async throws {
        let builds = BuildCounter()
        let wire = try await wire(builds)
        try wire.send(ref: "1", topic: "room:lobby", event: "alula:join")
        #expect(try await wire.nextEnvelope()?.event == "alula:reply")
        wire.close()
    }
    ...
    func joinStillGatesData() async throws {
        ...
        let builds = BuildCounter()
        let wire = try await wire(builds)
        try wire.send(ref: "1", topic: "room:private", event: "alula:join")
    ...
    func refusalReleasesTheTopic() async throws {
        ...
        try wire.send(ref: "2", topic: "admin:settings", event: "alula:join")
        #expect(try await wire.nextEnvelope()?.payload["reason"] == .string("forbidden"))
        wire.close()
    }
}
