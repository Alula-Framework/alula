...
struct PacedModule: AlulaModule {
...
private func replies(_ wire: ChannelWireClient, count: Int) async throws -> [String] {
    ...
    while labels.count < count {
        ...
        guard envelope.event == "alula:reply",
...
struct EnvelopeDispatchTests {
    ...
    func topicsRunConcurrently() async throws {
        ...
        try wire.send(ref: "j1", topic: "paced:a", event: "alula:join")
        try wire.send(ref: "j2", topic: "paced:b", event: "alula:join")
    ...
    func oneTopicStaysOrdered() async throws {
        let harness = try DispatchHarness(maxConcurrent: 8)
        let wire = try await harness.wire()
        try wire.send(ref: "j", topic: "paced:a", event: "alula:join")
        _ = try await wire.nextEnvelope()
    ...
    func inFlightIsBounded() async throws {
        ...
        let wire = try await harness.wire()
        for index in 0..<5 {
            try wire.send(ref: "j\(index)", topic: "paced:t\(index)", event: "alula:join")
        }
    ...
    func rejectedJoinIsNotAddressable() async throws {
        let harness = try DispatchHarness(maxConcurrent: 8)
        let wire = try await harness.wire()
        try wire.send(ref: "j", topic: "paced:locked", event: "alula:join")
        let rejection = try await wire.nextEnvelope()
        #expect(rejection?.event == "alula:error")
        ...
        #expect(refused?.event == "alula:error")
        #expect(refused?.payload["reason"] == .string("not_joined"))

        try wire.send(ref: "j2", topic: "paced:locked", event: "alula:join")
...
private struct CountingModule: AlulaModule {
    let channels: [ChannelRegistration] = [
...
struct TopicBoundTests {
    ...
    func boundIsEnforced() async throws {
        ...
        for index in 0..<3 {
            try wire.send(ref: "\(index)", topic: "many:\(index)", event: "alula:join")
        ...
        try wire.send(ref: "over", topic: "many:4", event: "alula:join")
        let refused = try await wire.nextEnvelope()
        #expect(refused?.event == "alula:error")
    ...
    func leavingFreesASlot() async throws {
        ...
        try wire.send(ref: "1", topic: "many:a", event: "alula:join")
        _ = try await wire.nextEnvelope()
        try wire.send(ref: "2", topic: "many:b", event: "alula:join")
        #expect(try await wire.nextEnvelope()?.payload["reason"] == .string("too_many_topics"))

        try wire.send(ref: "3", topic: "many:a", event: "alula:leave")
        _ = try await wire.nextEnvelope()
        try wire.send(ref: "4", topic: "many:b", event: "alula:join")
    ...
    func burstsAreBounded() async throws {
        ...
        for _ in 0..<10 {
            guard let envelope = try await wire.nextEnvelope() else { break }
            if envelope.event == "alula:reply" { accepted += 1 }
