...
Nothing here imports `AlulaWeb`. `RateLimiting` middleware is one consumer
...
service limit something without an HTTP server, the same way `AlulaSessions`
sits below both `AlulaWeb` and `AlulaSecurityCore`.
