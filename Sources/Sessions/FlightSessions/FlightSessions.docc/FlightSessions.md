...
`FlightSessionsValkey` in flight-data implements the seam over Valkey for
deployments with more than one.

This target has no dependencies and no HTTP in it. The middleware, the
cookie, and `context.session` are `FlightWeb`'s, which depends on this
target the way `FlightChannels` depends on `FlightPubSub`.
