...
public struct PasswordAuthenticator: Sendable {
    ...
    ///     `AlulaRateLimitModule`, so a Valkey-backed store limits across
    ///     replicas.
    ...
    public init(
        ...
        logger: Logger = Logger(label: "alula.security.password")
    ) {
        self.store = store
        self.issuer = issuer
        self.hasher = hasher
        ...
        do {
            if let clientAddress {
                let decision = try await limiter.consume(
                    "alula.sign-in.address:\(clientAddress)", quota: throttle.perAddress)
            ...
            let decision = try await limiter.consume(
                "alula.sign-in.identifier:\(identifier.lowercased())",
...
private final class DummyHash: Sendable {
    ...
    var value: String? {
        ...
        guard let hash = try? hasher.hash("alula.dummy-password") else { return nil }
