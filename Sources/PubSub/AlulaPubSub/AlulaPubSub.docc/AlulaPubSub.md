...
**An adapter ships in alula-data.** `AlulaPubSubValkey`'s
`ValkeyPubSubAdapter` implements ``DistributedPubSubAdapter`` over Valkey, and
`AlulaPubSubValkeyModule` wires it, so a clustered deployment is configuration
...
small. `AlulaPubSubTesting`'s `InMemoryCluster` is a second conforming
...
That is why `AlulaChannels` can build broadcast on top of this and stay
