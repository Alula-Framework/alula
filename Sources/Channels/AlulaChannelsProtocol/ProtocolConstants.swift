...
public enum ReservedEvent: String, Sendable, CaseIterable {
    ...
    /// with this; the server answers such an event with `alula:error`.
    public static let prefix = "alula:"
}

/// Protocol-level names and values shared by server and clients.
public enum ChannelProtocol {
    /// The topic socket-level control events (`alula:heartbeat`,
    /// `alula:close`) travel on. Reserved: it can never be joined.
    public static let controlTopic = "alula"

    /// The `alula:error` payload is `{"reason": <string>}`; this is its key.
    ...
    /// namespaced its gossip (`alula:presence`), and channel traffic now
    /// does the same.
    public static let busPrefix = "alula:channels:"
...
/// Error reasons the server emits in `alula:error` payloads. Strings, not
...
public enum ChannelErrorReason {
    ...
    /// A client-sent event was `alula:`-namespaced but is not one a client
    /// may send (`alula:reply`, `alula:error`, unknown `alula:*`).
    public static let invalidEvent = "invalid_event"
}

/// WebSocket close codes Alula Channels uses beyond the RFC 6455 standard
...
public enum ChannelCloseCode {
    ...
    /// Alula owns both clients, this is always a bug or an attack — the
