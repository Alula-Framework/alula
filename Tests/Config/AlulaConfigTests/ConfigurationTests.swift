...
struct ConfigurationTests {
    ...
    func keyByKeyMerge() throws {
        ...
        let base = TestConfigSource([
            "datasource.url": "postgres://localhost:5432/alula_dev",
    ...
    func missingKeyMessage() {
        ...
        do {
            ...
            #expect(message.contains("ALULA_DATASOURCE_URL"))
        }
    }
    ...
    func valuesConvenience() throws {
        let config = Configuration(values: ["greeting.name": "alula"])
        #expect(try config.get("greeting.name", as: String.self) == "alula")
