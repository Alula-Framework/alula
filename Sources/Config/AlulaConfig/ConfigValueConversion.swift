...
extension ConfigContent {
    ...
    /// `AlulaYAMLSnapshot` — as two near-identical switches differing only
    ...
    /// `AlulaYAMLSnapshot` has. The caller decides what nil means and what
    /// to throw, which is the only thing that ever differed.
    static func alulaScalar(_ raw: String, as type: ConfigType) -> ConfigContent? {
        switch type {
...
extension ConfigType {
    ...
    var alulaTargetTypeName: String {
        switch self {
        case .string: "String"
        case .int: "Int"
        case .double: "Double"
...
/// three-layer stack was thirty. Alula's own providers know their key set
...
internal protocol AlulaKeyPresenceProvider {
