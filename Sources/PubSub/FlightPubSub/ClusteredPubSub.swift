...
public final class ClusteredPubSub: PubSub, Sendable {
    ...
    ///     nodes and delivers theirs back. flight-data ships one,
    ///     `FlightPubSubValkey`'s `ValkeyPubSubAdapter`; the protocol is two
    ///     methods, narrow enough to write against NATS or Redis too, and
    ///     `FlightPubSubTesting`'s `InMemoryCluster` is a second conforming
