...
struct LoaderProviderTests {
    ...
    func additionalProvidersWin() throws {
        try withConfigDirectory(files: ["alula.yaml": baseYAML]) { directory in
            let config = try Configuration.load(
                from: directory,
                processEnvironment: ["ALULA_DATASOURCE_POOL_SIZE": "50"],
            ...
            #expect(
                try config.get("datasource.url", as: String.self)
                    == "postgres://localhost:5432/alula_dev"
            )
        }
    }
    ...
    func envLayerEncoding() throws {
        try withConfigDirectory(files: ["alula.yaml": baseYAML]) { directory in
            ...
            // prefixKeys(with: "alula") must reproduce
    ...
    func secretsRedact() throws {
        try withConfigDirectory(files: ["alula.yaml": baseYAML]) { directory in
            let config = try Configuration.load(
                from: directory,
                processEnvironment: ["ALULA_DATASOURCE_PASSWORD": "hunter2"],
                secrets: .specific(["ALULA_DATASOURCE_PASSWORD"])
            )
            ...
            #expect(dump.contains("ALULA_DATASOURCE_PASSWORD=<REDACTED>"))
    ...
    func accessReporting() throws {
        ...
        try withConfigDirectory(files: ["alula.yaml": baseYAML]) { directory in
            ...
            // Reporting is a reader-side facility: Alula's own accessors go
