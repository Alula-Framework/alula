...
/// from what `AlulaWebModule` was composed with. Anything a handler
...
public struct RequestContext: Sendable {
    ...
    ///
    /// Alula Web stores it and never reads it: the seam exists so an
    ...
    /// `AlulaWebModule` was composed with (§2.5), replacing a container
    /// lookup.
