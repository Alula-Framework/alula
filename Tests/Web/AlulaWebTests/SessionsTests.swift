...
struct SessionsTests {
    ...
    private func client(
        ...
        return try TestClient(
            routes: SessionTestController.alulaRoutes { _ in SessionTestController() },
    ...
    func notConfigured() async throws {
        ...
        #expect(response.status == .internalServerError)
        #expect(!response.bodyText.contains("AlulaSessionsModule"))
        #expect(SessionNotConfiguredError().description.contains("AlulaSessionsModule"))
    }
...
@Suite("AlulaSessionsModule")
struct AlulaSessionsModuleTests {
    ...
    func adapter() throws {
        ...
        let module = try AlulaSessionsModule(configuration: Configuration(), store: store)
    ...
    func unloadedAdapter() throws {
        ...
        #expect(throws: UnloadedAdapterError.self) {
            try AlulaSessionsModule(configuration: configuration)
        }
        do {
            _ = try AlulaSessionsModule(configuration: configuration)
        } catch let error as UnloadedAdapterError {
            #expect(error.module == "AlulaSessionsValkeyModule")
    ...
    func settings() throws {
        let module = try AlulaSessionsModule(
    ...
    func bounds() throws {
        ...
        #expect(throws: SessionConfigurationError.nonPositiveTTL(.zero)) {
            try AlulaSessionsModule(configuration: Configuration(values: ["sessions.ttl": "0s"]))
        }
    }
