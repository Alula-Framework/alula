...
/// Alula-subset YAML as a swift-configuration snapshot.
///
/// This is the seam that puts Alula's parser inside Apple's provider
...
///
/// ```swift
/// FileProvider<AlulaYAMLSnapshot>(filePath: "alula.yaml")           // immutable
/// ReloadingFileProvider<AlulaYAMLSnapshot>(filePath: "alula.yaml")  // polling
/// ```
///
/// The subset grammar, the `${VAR}` rules, and every "this is outside the
/// subset" error are unchanged — they come from `AlulaConfigCore`, the same
/// code the build plugin runs, so build-time and runtime still cannot
/// disagree about what a file says.
///
/// ## Values are string-native
///
/// Alula's parser flattens a document to `[String: String]` — YAML's typing
/// rules are deliberately not applied. A snapshot therefore answers a request
/// for *any* `ConfigType` by converting the raw string, which means Alula's
...
public struct AlulaYAMLSnapshot: FileConfigSnapshot {

    /// Parsing knobs. `${VAR}` substitution is the only one Alula has, and
    ...
    public init(document: AlulaYAMLDocument, providerName: String) {
        self.providerName = providerName
    ...
    public var description: String {
        "AlulaYAML[\(providerName), \(document.keys.count) keys]"
    }
...
extension ConfigValue {
    ...
    fileprivate init(
        ...
        guard let content = ConfigContent.alulaScalar(raw, as: type) else {
            throw AlulaYAMLConversionError(key: key, rawValue: raw, type: type)
        }
    ...
    fileprivate init(
        ...
        func mapped<T>(_ transform: (String) -> T?) throws -> [T] {
            try elements.map { element in
                guard let value = transform(element) else {
                    throw AlulaYAMLConversionError(key: key, rawValue: element, type: type)
                }
                return value
            }
        }
        ...
        default:
            throw AlulaYAMLConversionError(key: key, rawValue: elements.joined(separator: ","), type: type)
        }
    }
}
...
struct AlulaYAMLConversionError: Error, CustomStringConvertible {
    let key: String
    let rawValue: String
    let type: ConfigType
