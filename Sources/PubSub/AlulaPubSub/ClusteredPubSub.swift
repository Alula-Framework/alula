...
public final class ClusteredPubSub: PubSub, Sendable {
    ...
    /// Metadata keys under this prefix (`alula.local.`) are delivered to
    ...
    /// value means.
    public static let localOnlyMetadataPrefix = "alula.local."

    /// Reserved metadata key (`alula.pubsub.origin`) carrying the
    ...
    public static let originMetadataKey = "alula.pubsub.origin"

    /// Reserved metadata key carrying this *instance's* token, which is what
    /// echo suppression actually matches on. See ``nodeID``.
    public static let instanceMetadataKey = "alula.pubsub.instance"
    ...
    ///     nodes and delivers theirs back. alula-data ships one,
    ///     `AlulaPubSubValkey`'s `ValkeyPubSubAdapter`; the protocol is two
    ///     methods, narrow enough to write against NATS or Redis too, and
    ///     `AlulaPubSubTesting`'s `InMemoryCluster` is a second conforming
    ///     implementation used to test the clustered paths.
    ...
    public init(
        ...
        logger: Logger = Logger(label: "alula.pubsub.clustered")
    ) {
        self.local = local
    ...
    private func broadcast(_ message: Message) async throws {
        ...
        try await withAlulaTimeout(broadcastTimeout, throwing: BroadcastTimeout()) {
