    ...
    func untrustedServerCertificateFails() async throws {
        try await withMaterial { serverTLS, _, _, _ in
            ...
            let otherDirectory = URL(fileURLWithPath: NSTemporaryDirectory())
                .appendingPathComponent("alula-tls-other-\(UUID().uuidString)")
...
struct TLSConfigurationTests {
    ...
    private func configuration(_ pairs: [String: String]) -> AlulaCore.Configuration {
        AlulaCore.Configuration(values: pairs)
    }

    @Test("no TLS keys means plaintext, not an error")
    func absentMeansPlaintext() throws {
        let transport = try AlulaTransportConfiguration(configuration: configuration([:]))
    ...
    func halfConfiguredIsAnError() throws {
        #expect(throws: TLSConfigurationError.self) {
            try AlulaTransportConfiguration(
    ...
    func bothKeysEnableTLS() throws {
        let transport = try AlulaTransportConfiguration(
        ...
        #expect(transport.tls?.clientAuthentication == AlulaTransportConfiguration.TLS.ClientAuthentication.none)
    }
    ...
    func clientAuthNeedsRoots() throws {
        let tls = AlulaTransportConfiguration.TLS(
