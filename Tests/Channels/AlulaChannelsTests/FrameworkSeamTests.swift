@_spi(AlulaInternal) import AlulaChannels
...
/// The `@_spi(AlulaInternal)` seams sibling framework packages (Presence,
...
struct FrameworkSeamTests {
    ...
    struct SeamChannel: Channel {
        ...
        func handle(_ event: InboundEvent, socket: Socket) async -> HandleResult {
            if event.event == "push_reserved" {
                socket.pushReserved(topic: event.topic, event: "alula:presence_state", payload: ["who": "me"])
                return .none
            }
            ...
            // `{"type": "alula:anything"}` reached a `precondition`.
    ...
    func reservedNameFromPayloadDoesNotKillTheProcess() async throws {
        ...
        // bad name.
        //
        // The framework filters `alula:`-prefixed events arriving in an
        ...
        try wire.send(ref: "1", topic: "seam:a", event: "alula:join")
        _ = try await wire.expectEnvelope { $0.ref == "1" }

        try wire.send(
            ref: "2", topic: "seam:a", event: "echo_type",
            payload: ["type": "alula:presence_state"])
    ...
    func terminationOnLeave() async throws {
        ...
        try wire.send(ref: "1", topic: "seam:a", event: "alula:join")
        try wire.send(ref: "2", topic: "seam:b", event: "alula:join")
        try wire.send(ref: "3", topic: "seam:a", event: "alula:leave")
    ...
    @Test("pushReserved delivers an alula:* event to exactly this socket")
    func pushReserved() async throws {
        ...
        try wire.send(ref: "1", topic: "seam:a", event: "alula:join")
        try bystander.send(ref: "1", topic: "seam:a", event: "alula:join")
        _ = try await wire.nextEnvelope()
        ...
        #expect(received?.event == "alula:presence_state")
        ...
        while let envelope = try await bystander.nextEnvelope() {
            #expect(envelope.event != "alula:presence_state")
        }
    }
}
