...
struct SubstitutionTests {
    ...
    @Test("the design doc's alula-prod.yaml example resolves via the env")
    func designDocExample() throws {
        let source = try resolve("""
        datasource:
          url: "${ALULA_DATASOURCE_URL}"
          pool_size: 50
        """, env: ["ALULA_DATASOURCE_URL": "postgres://prod-db:5432/alula"])
        #expect(source.rawValue(for: "datasource.url") == "postgres://prod-db:5432/alula")
