...
//
// These expected-output strings ARE the specification of Alula's macro
...
//       container-era `init(_alula:)`, the `_alulaRegister` thunk and the
//       `_AlulaRegistrable` conformance all went with the container
...
//       qualifiers are a compile error (fixture 6a) — Alula refuses to
...
struct MacroFixtureTests {
    ...
    func repositoryStereotype() {
        ...
        // migration is the generator's, and AlulaRegistrationGenTests'
    ...
    func qualifiedInjectIsRefusedToo() {
        ...
        // alula-registration-gen, which no macro-expansion fixture can
    ...
    // the parameter — `_alulaConfiguration` — and only when the type reads
        ...
        assertMacroExpansion(
            ...
            expandedSource: """
                final class ServerSettings {
                    ...
                    init(_alulaConfiguration configuration: AlulaCore.Configuration) throws {
    ...
    // MARK: Supplementary — @ConfigValue with default: (Alula Config §5)
    //
