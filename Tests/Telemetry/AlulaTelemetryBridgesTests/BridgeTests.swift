    ...
    extension CoreTests {
        ...
        struct TelemetryModuleTests {
            ...
            func module() throws {
                let metrics = TestMetrics()
                let module = try AlulaTelemetryModule(
            ...
            func autoOff() throws {
                ...
                let forced = try AlulaTelemetryModule(
            ...
            func settings() throws {
                ...
                let custom = try TelemetrySettings(
                    configuration: Configuration(values: [
                        ...
                        "telemetry.log.prefix": "alula.sessions", "telemetry.log.level": "INFO",
                    ]))
                #expect(custom.tracingEnabled == true && custom.tracingPrefix == "hangar")
                #expect(custom.logPrefix == "alula.sessions" && custom.logLevel == .info)

                #expect(
                    throws: TelemetryConfigurationError.invalidPrefix(
                        key: "telemetry.log.prefix", value: "Alula")
                ) {
                    _ = try TelemetrySettings(
                        configuration: Configuration(values: ["telemetry.log.prefix": "Alula"]))
                }
